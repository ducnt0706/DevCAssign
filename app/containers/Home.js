...
export default function Home() {
  ...
  const searchByTitle =()=>{
    const searchVal=searchField.toUpperCase();

    const dataVal=articles[0].title.toUpperCase();
    if(dataVal.indexOf(searchVal)>3){
      alert(dataVal);
    }
    //alert();
    ...
    <TouchableWithoutFeedback
      ...
      <View style={globalStyles.containerBody}>
        ...
        <View style={globalStyles.horizontalContainer}>
          <TextInput style={globalStyles.textInput} placeholder="  e.g covid19" onChangeText={(val)=>setSearchField(val)}/>
